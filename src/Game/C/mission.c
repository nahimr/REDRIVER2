...
void LoadMission(int missionnum)
{
	...
	PsyX_Log_Info("MissionNum: %d\n", missionnum);
	PsyX_Log_Info("MissionLoadAddress: %d\n", MissionLoadAddress->size);

	MissionTargets = (MS_TARGET*)((char*)MissionLoadAddress + MissionLoadAddress->size);

	MissionScript = reinterpret_cast<u_int*>(MissionTargets + MAX_MISSION_TARGETS);
	...
	switch(target->type)
	{
		...
		case Target_Event: // event target
		{
			if (target->s.target_flags & TARGET_FLAG_EVENT_TRIGGERED)
			{
				...
				if (target->s.event.loseMessage != -1 && Long2DDistance((*(VECTOR**)&target->s.event.eventPos), &pv) > 3000)
				{
			...
			{
				// target->s.event.eventPos = reinterpret_cast<uint32_t*>(TriggerEvent(target->s.event.eventId));
