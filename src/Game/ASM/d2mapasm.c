...
sdPlane* sdGetCell_asm(int param_1, short** roadMapData, int pos_x, int pos_z, int pos_y, int _sdLevel)
{
	...
	buffer = *(short**)((param_1 ^ sdSelfModifyingCode) + (s_int64_t)roadMapData);

	cell.x = pos_x & 1023;
	cell.y = pos_z & 1023;

	surface = (ushort*)((s_int64_t)buffer + (pos_z >> 3 & 0x1f80U) + (pos_x >> 9 & 0x7eU) + 8);
	...
	else
	{
		...
		{
			HSurface = (short*)((s_int64_t)buffer + (*surface & 0x1fff) * 2 + (int)buffer[2]);
		...
		do
		{
			...
			if ((*surface & 0x4000) != 0)
			{
				BSPSurface = (ushort*)sdGetBSP((_sdNode*)((s_int64_t)buffer + (*surface & 0x1fff) * 4 + (int)buffer[3]), &cell);
		...
		plane = (sdPlane*)((s_int64_t)buffer + (short)*BSPSurface * sizeof(sdPlane) + (int)buffer[1]);
	}
